...
Soni_Choice_Trad = traditionalSoni(hslider("Choice_Traditional",0,0,1,0.001)) : si.smoo; 

dummySoniGroup = Soni_X_B1_MelBaseFreq,Soni_X_B2_MelLoudness,Soni_X_B4_MelBrightness,Soni_X_B5_PercRelTime,Soni_X_R1_InstSynch,Soni_X_R2_Inharmonicity,Soni_X_R3_Tremolo,Soni_X_R4_Dist,Soni_X_RZ1_MultiPhaser,
Soni_X_RZ2_MultiWah,Soni_X_Trad,Soni_Choice_Trad :> _;
...
Soni_B2_A_Max = 0;	Soni_B2_A_Min = -40;
...
Soni_RZ_OscFreqs(index) = tempo/60, tempo/30, tempo/15 : select3(index);
Soni_RZ_LFO_Riff = os.osc(Soni_RZ_OscFreqs(0));		Soni_RZ_LFO_Chords = os.osc(Soni_RZ_OscFreqs(1));		Soni_RZ_LFO_Mel = os.osc(Soni_RZ_OscFreqs(2)); 
Soni_RZ1_numNotches = 10; 							Soni_RZ1_notchWidth = 200; 								Soni_RZ1_notch1Min = 100;
Soni_RZ1_notchSpacing = 1.5;						Soni_RZ1_notch1Max = 150;
...
Soni_B4_Filter = fi.resonlp(cutoff,2.5,1) with {cutoff = Soni_B4_Fc_Max - pow(Soni_X_B4_MelBrightness,0.2) * (Soni_B4_Fc_Max - Soni_B4_Fc_Min);};
Soni_B5_RelFactor = 1 + Soni_X_B5_PercRelTime * Soni_B5_MaxRelFactor;
Soni_R1(maxDelay) = _ : de.fdelay(50000,cookedDelaySamples) with {cookedDelaySamples = maxDelay * Soni_X_R1_InstSynch * ma.SR;};
...
Soni_R3_TremoloFX = _ <: *(mult * tremoloLFO),*(1-mult) :> _ with { tremoloLFO = 0.5*(1 + os.osc(tempo/30)); mult = pow(Soni_X_R3_Tremolo,0.4); };
Soni_R4_Dist = _ <: ef.cubicnl(0.8,0)*(mult),*(1-mult) :> _ with { mult = pow(Soni_X_R4_Dist,0.3) * (1 - 0.5*Soni_X_R4_Dist); };
Soni_RZ1_Phaser(oscFreq) = _ : pf.phaser2_mono(Soni_RZ1_numNotches,0,Soni_RZ1_notchWidth,Soni_RZ1_notch1Min,Soni_RZ1_notchSpacing,Soni_RZ1_notch1Max,oscFreq,(1 - Soni_X_RZ1_MultiPhaser),0,0);
Soni_RZ2_Wah(LFO,minFreq,maxFreq) = _ : fi.peak_eq_cq(currentGain,currentFreq,0.7) : *(ba.db2linear(-0.4*currentGain)) with
{
  ...
  currentGain = 8 * pow((1 - Soni_X_RZ2_MultiWah),0.4);
};
// // // // // // // // // // // // // // // // // // // // // // // // // // // // // // Generation // // // // // // // // // // // // // // // // // // // // // // // // // // // // // // 
...
melodySynth_ChosenStyle = melodySynth_S1, melodySynth_S2, melodySynth_S3, melodySynth_S4 : ba.selectn(numStyles,currentStyle) : Soni_R1(Soni_R1_MaxDelSec(5)) : Soni_RZ1_Phaser(Soni_RZ_OscFreqs(2)) : Soni_RZ2_Wah(Soni_RZ_LFO_Mel,Soni_RZ2_wahMinFreq,Soni_RZ2_wahMaxFreq);
...
crashSynth_SONIDUMMY = attach(crashSynth_ChosenStyle,dummySoniGroup);

crashTrack = crashSynth_SONIDUMMY : channelComp(compTrackGroup(8)) : parametricEQ(eqTrackGroup(8)) : getPanFunction(2);
